...
var bunnyhop = module.exports = function(config) {
	var exchangeName 	= config.exchange; 							//'testexchange';
	var queueName 		= config.queue; 								//'testqueue';
	var routingKey 		= config.routingKey; 						//'test.route';
	var restEndpoint 	= config.restEndpoint; 					//'http://localhost:3000';
	var maxRetry			= parseInt(config.retryCount); 	//5;
	var retryDelay 		= parseInt(config.retryDelay); 	//100; // it will double after each retry
	console.log(config);
